...
/**
 * Minimal headless {@link IGuiBase} so Forge can initialise without a real GUI.
 * Copied from Forge's own ForgeMatrixWriter tool (GPL-3.0) — every UI/asset
 * method is a no-op or a deterministic default; choice methods pick the first
 * option (they are only hit if a decision reaches the GUI rather than a
 * PlayerController, which won't happen in our bridge).
 */
public final class HeadlessGui implements IGuiBase {
    private final String assetsDir;

    public HeadlessGui(Path forgeGuiDir) {
    ...
    // true so GuiBase.isNetPlay() short-circuits FThreads.assertExecutedByEdt(...):
    // this lets Forge's input system (InputProxy / Input.showAndWait) block our
    // headless game thread on its latch instead of throwing an EDT assertion.
    // See DESIGN.md §13.3.
